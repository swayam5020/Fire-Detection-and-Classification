 ...
 * bundled world-atlas country geometry, same light fill/outline colors),
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: {
        version: 8,
        sources: {},
        layers: [{ id: 'bg', type: 'background', paint: { 'background-color': '#e6ebf1' } }],
      },
    ...
    map.on('load', async () => {
      try {
        ...
        map.addLayer({ id: 'countries-fill', type: 'fill', source: 'countries', paint: { 'fill-color': '#d3d8e0' } });
        map.addLayer({
    ...
    wrapper.innerHTML = `
      ...
      <span class="relative block h-3 w-3 rounded-full border-2 border-white" style="background-color:${color};box-shadow:0 1px 3px rgba(18,21,26,0.4)"></span>
    `;
